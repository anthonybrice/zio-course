...
object Refs extends ZIOAppDefault {
  ...
  def tickingClockImpure: UIO[Unit] =
    var ticks = 0L
    // print the current time of the system every 1s + increase a counter ("tick")
  ...
  def tickingClockPure: UIO[Unit] =
    ...
    def printTicks(ticks: Ref[Long]): UIO[Unit] = for
      _ <- ZIO.sleep(5.seconds)
      currTicks <- ticks.get
      _ <- ZIO.succeed(s"TICKS: $currTicks").debugThread
      _ <- printTicks(ticks)
    yield ()

    Ref.make(0L).flatMap(ticks => tickingClock(ticks) zipPar printTicks(ticks))

  // 2
  def tickingClockPure_v2: UIO[Unit] =
    val ticksRef: UIO[Ref[Long]] = Ref.make(0L) // ZIO is lazy, gets produced in each for comprehension
    // print the current time of the system every 1s + increase a counter ("tick")
    def tickingClock: UIO[Unit] = for
      ticks <- ticksRef
      _ <- ZIO.sleep(1.second)
