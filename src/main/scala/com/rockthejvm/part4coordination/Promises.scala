...
object Promises extends ZIOAppDefault:
  ...
  def eggBoiler: Task[Unit] =
    def boilEgg(r: Ref[Int], p: Promise[Throwable, Unit]): Task[Unit] = for
      _ <- ZIO.sleep(1.second)
      t <- r.updateAndGet(_ + 1)
      _ <- if t == 10 then p.succeed(()) else boilEgg(r, p)
    yield ()

    def chime(p: Promise[Throwable, Unit]): Task[Unit] = for
      _ <- p.await
      _ <- ZIO.succeed("Ringing bell!!").debugThread
    yield ()

    for
      r <- Ref.make(0)
      p <- Promise.make[Throwable, Unit]
      _ <- boilEgg(r, p) zipPar chime(p)
    yield ()
  ...
  def racePair[R,E,A,B](
    ...
    ZIO.uninterruptibleMask { restore => for
      p <- Promise.make[Nothing, Either[Exit[E,A], Exit[E,B]]]
      fiba <- restore(zioa).onExit(exita => p.succeed(Left(exita))).fork
      fibb <- restore(ziob).onExit(exitb => p.succeed(Right(exitb))).fork

      result <- restore(p.await).onInterrupt {
        for
