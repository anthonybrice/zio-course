...
object Fibers extends ZIOAppDefault {
  ...
  def runOnAnotherThread_v2[R,E,A](zio: ZIO[R,E,A]) = for {
    ...
    case Exit.Success(v) => s"succeeded with $v"
  ...
  // 1 - zip 2 fibers without zip combinator
  ...
  def chainFibers[E,A](fiber1: Fiber[E,A], fiber2: Fiber[E,A]): ZIO[Any, Nothing, Fiber[E,A]] = for {
    ...
    case Exit.Success(_) => fiber1
    case Exit.Failure(_) => fiber2
  ...
  def countWords(n: Int): ZIO[Any, Throwable, Int] = for {
    fibs <- ZIO.collectAll((1 to n).map(i => ZIO.attempt(countWordsInFile(i)).fork))
    words <- Fiber.collectAll(fibs).join
  } yield words.sum

  def countWordsInFile(n: Int) = {
    val content = Files.readString(new File(s"src/main/resources/testfile_$n.txt").toPath)
